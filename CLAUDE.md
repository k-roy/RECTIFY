...
This has been validated repeatedly on real DRS data (mini-1k sidecar, chunk003 preflight).
...
`split_command.py` now implements correct-before-consensus order (fixed 2026-04-30).
The old `_merge_consensus_body()` that called `rectify consensus` on raw BAMs has been
...
Pass `--short-read` to generate a simplified two-stage pipeline for Illumina
short reads (QuantSeq REV, dT-primed cDNA):
...
Generated scripts:
- `run_array_short_read.sh` (array 0..N-1): align (bbmap+bwa) → correct → consensus per chunk, all I/O on `$SCRATCH`
- `run_final_merge_short_read.sh`: merge chunks → sort → index → copy to `--oak-output-dir` → clean up scratch
- `submit_pipeline.sh`: chains both with `--dependency=afterok`

**I/O policy in generated scripts:** chunk FASTQs and all intermediate BAMs write to
`$SCRATCH`. Only the final merged BAM is copied to Oak. Do NOT change intermediate
output paths to point at Oak — BGZF-compressed BAM writes to NFS run at ~1 MB/min;
`$SCRATCH` runs at ~75 GB/s aggregate bandwidth.

**Differences from long-read pipeline:**
- Aligners: `bbmap + bwa` instead of `mapPacBio/minimap2/gapmm2/uLTRA/deSALT`
- No prescan step (no junction pool)
- `--dT-primed-cDNA` passed to all `rectify correct` calls — AG-mispriming module **is enabled** for short reads (genomic-context check downstream of 3' end)
- Two scripts instead of seven

---

## ⚠️ SCRATCH STAGING: Long-Read Array Scripts Need Manual Patching (CRITICAL)

**The long-read pipeline scripts generated by `rectify split --generate-slurm` write alignment
outputs directly to `$OUTDIR` (Oak NFS) by default. This MUST be patched before running
at scale.**

### Why this matters

100+ parallel array tasks all writing BAMs to Oak NFS simultaneously causes severe I/O
contention. BGZF-compressed BAM writes to NFS run at ~1 MB/min. `$SCRATCH` runs at
~75 GB/s aggregate bandwidth — orders of magnitude faster.

**Symptom:** Jobs appear to run (CPU active) but produce no output or outputs accumulate very
slowly. The bottleneck is Oak NFS, not computation.

### The patch (apply to every generated array script)

```bash
# === ADD SCRATCH STAGING ===
# Replace the direct $OUTDIR write pattern:
#   CHUNK_OUTDIR="$OUTDIR/aligner_chunks/mapPacBio/chunk_${CHUNK_PAD}"
#   mkdir -p "$CHUNK_OUTDIR"
#   rectify align ... --output "$CHUNK_OUTDIR/..."
#
# With this scratch-staging pattern:
...
Use suffix `rectify_mpb_` for `run_array_mapPacBio.sh` and `rectify_${ALIGNER}_` for
`run_array_others.sh`. Verify that ALL `rectify align` and `rectify correct` `--output` flags
point to `$SCRATCH_DIR`, not `$OAK_OUTDIR`.

### Scripts that need patching after `rectify split --generate-slurm`

| Script | Variable to rename | Scratch prefix |
|--------|--------------------|----------------|
...
`split_command.py` should be updated to generate scratch-staging by default for the long-read
array scripts (as it already does for short-read scripts via `--oak-output-dir`). Until that
is done, **always patch generated scripts before submitting at scale.**
...
**`samtools fastq -T pt` embeds the poly-A tag as a tab-separated suffix in the FASTQ header:**
```
@UUID\tpt:i:25
```
mapPacBio reads the entire header verbatim and writes `UUID_pt:i:25` as the BAM QNAME after
`samtools sort`. This creates duplicate read IDs (the same molecule appears as `UUID_pt:i:0`
and `UUID_pt:i:13`) and breaks parquet lookups that are keyed by bare UUID.

**Fix (implemented in `run_command.py`):** call `samtools fastq` without `-T pt`. Poly-A lengths
are stored in the parquet trim metadata — they do not need to be in the FASTQ header. The
FASTQ read name must always be the bare UUID.

Note: `preprocess.py`'s `align_dorado_bam_to_genome()` uses `samtools fastq -T '*' | minimap2 -y`
which is a different code path — it pipes directly into minimap2 without writing a FASTQ file, so
the tags go into minimap2 aux fields (not the read name) and are safe.
...
**`--short-read` disables only** poly(A) trimming and indel correction, which
require a sequenced poly(A) tail.

**A-tract walk-back (2E) is enabled for short reads** — CSP internal priming
in QuantSeq REV causes poly-A complement T's that, after reverse
complementation for alignment, appear as A's and slide into downstream genomic
A-runs, shifting the reported 3' end rightward.  `find_atract_boundaries()`
uses the reference genome, not the read sequence, so it corrects this
regardless of read length.  Specifically: reads ending in `AAAAAAAAAT` (A-run
terminated by a genomic T) are walked back to the last non-A position upstream
of the A-run.

**AG mispriming — why DRS is exempt:** DRS (Oxford Nanopore direct RNA) has no
priming step — the RNA is sequenced directly.  Oligo-dT mispriming cannot
occur.  The module is therefore off by default and enabled only with
`--dT-primed-cDNA`.

**AG mispriming — why short reads need it most:** For QuantSeq/Illumina dT-
primed data, the oligo-dT primer can misprime onto genomic A/G-rich regions,
producing false 3' end sites.  The reads stop at the cleavage site, so the
A/G-rich genomic context is **downstream of the reported 3' end** — absent
from the read itself.  The module detects mispriming by calling
`get_downstream_sequence()` on the genome, not by inspecting read sequence.
This genomic-context approach works identically for short and long reads, and
short-read QuantSeq data is the primary use case.
...
`corrected_reads.tsv`. This is a pre-aggregated position count file:
...
Stored in `corrected_reads.tsv` as the `five_prime_exon_cigar` column (v2.8.0).
...
│   │   ├── position_index.py # Writes corrected_reads_index.bed.gz
...
New `five_prime_exon_cigar` column in `corrected_reads.tsv`. Chimeric stitch
...
**v3.3.0 (2026-05-02):** Chimeric-junction overhang filter (`--junction-overhang-table`) + two critical `corrected_consensus.py` correctness fixes:

- **New module: `calibrate_junction_overhang.py`** — empirically calibrates the minimum-overhang threshold per intron-size bin from concordant multi-aligner reads (≥4 aligners agree on the junction). Produces `junction_overhang_table.tsv`. Uses isotonic regression (PAV) to enforce monotone non-decreasing thresholds. Example production table in `common/scripts/nanopore/overhang_table_20260502/`.
- **New chimera filter in `merge_corrected_tsvs`** — `_add_chimera_flag(rep_df, overhang_table, junction_stats)` sets `_chimera_ok=1` for aligner rows whose junction has insufficient flanking overhang for its intron size. Chimeric aligners are sorted AFTER non-chimeric ones; if all aligners are chimeric, HP edit distance resolves the tie (graceful degradation). Short-intron relaxation (< 500 bp, ≥5 cross-read support, max_overhang ≥ 10 bp) allows 1 bp minimum overhang for well-supported junctions. Wired into `rectify run-all` via `--junction-overhang-table`.
- **FIX: `index_col=False` in `_load_tsv`** — minimap2/gapmm2/uLTRA corrected TSVs are written with `index=True` (32 header columns, 33 data columns). Without `index_col=False`, pandas auto-uses the first data column (UUID) as the row index and shifts ALL column names one right: `read_id` → chromosome, `chrom` → strand, etc. With `index_col=False`, column mapping is correct. The root cause: these three aligners write TSVs via `pd.DataFrame.to_csv(index=True)` with an unnamed index, which pandas reads back as a header/data length mismatch. deSALT and mapPacBio write with `index=False` (or with a named index column) and are unaffected. **Impact**: in any merge that ran before this fix, gapmm2/minimap2/uLTRA read_ids were chromosome names (~17 unique values), so those aligners NEVER properly competed with mapPacBio/deSALT in winner selection. The original chunk003_correct_test merged TSV (pre-fix) was generated with this bug active.
- **FIX: `_normalize_read_id` handles both `_pt:i:N` forms** — the mapPacBio corrected TSV stores read names as `UUID_pt:i:N` (underscore-separated, from `samtools sort`), but the previous `_normalize_read_id` only stripped the space-separated form (`UUID pt:i:N`). After the fix, both space and underscore forms are stripped to bare UUID before the merge join. Without this fix, mapPacBio rows would have `UUID_pt:i:N` read_ids that didn't match any other aligner's `UUID` read_ids, causing mapPacBio to win 100% of reads from its own isolated groups (no competition) rather than competing on HP edit distance.
...
- **Full pipeline re-run**: `corrected_reads.tsv` (36 rows: 32 high + 4 chimeric), `rectified_corrected_3end.bam`, `rectified_pA_tail_trimmed.bam`, `rectified_pA_tail_soft_clipped.bam` all regenerated from the fixed validation_reads.bam.
...
- `corrected_reads.tsv` and rectified BAMs regenerated from DRS-trimmed validation set.
...
- Bundled validation outputs regenerated: `corrected_reads.tsv`, rectified BAMs, PROVENANCE.json all updated to reflect 36 reads.
...
- Cat2 rescue metadata (`sc_homopolymer_extension`, `sc_rescued_seq`, `sc_original_softclip_len`) now written to corrected_reads.tsv and read back by bam_writer.
