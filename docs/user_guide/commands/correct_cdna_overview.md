...
| Owner | Tags |
|-------|------|
| `rectify correct-cdna` (Stage 1) | `XU`, `XO`, `XC`, `XR`, `XM`, `XF`, `XA` (pre-align, sequence-level), `XT`, `XY`, `XQ`, `XK`, `XB`, `XN`, `XP` / `XD` / `XW` (dorado `pt` mean / n / SD, when the pre-aligned input carries `pt`) |
