...
Inter is embedded via `@font-face` in `svg_open()` pointing to the conda font path (`anaconda3/lib/lib/fonts/Inter-Regular.otf` and `Inter-SemiBold.otf`). This ensures consistent rendering on Sherlock where Helvetica Neue is not available via fontconfig. The `@font-face` block is injected into a `<defs><style>` element in every SVG.
...
**Rules:**
- One font family throughout (Inter, embedded via `@font-face`; Helvetica Neue/Arial as fallback)
...
First section:   y = title + 30
  ...
  Second row:    first row + 50–70 (genome-to-read gap; use 70 when labels between rows need clearance)
...
def save(name, svg_str, h):
    ...
    cairosvg.svg2png(
        url=svg_path,
        write_to=png_path,
        scale=2,    # 2x for retina (1520×Npx output from 760×Npx SVG)
    )
```
