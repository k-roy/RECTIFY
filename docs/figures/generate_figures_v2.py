...
@font-face {{
  font-family: 'Inter';
  src: url('{_FONT_DIR}/Inter-Regular.otf');
  font-weight: 400;
  font-style: normal;
}}
@font-face {{
  font-family: 'Inter';
  src: url('{_FONT_DIR}/Inter-SemiBold.otf');
  font-weight: 600;
  font-style: normal;
}}
"""
...
def fig_indel_correction():
    ...
    # T basecalling error — soft-clipped (S op), outside the aligned region
    # The aligner places a soft-clip here; reference_end stops at the preceding A
    sc_x = X0 + 14 * BW
    L.append(
        f'<rect fill="{PAL["orange_l"]}" height="{BH}" rx="{BR}" width="{BW}" '
        f'x="{sc_x}" y="{y_r}" stroke="{PAL["orange"]}" stroke-width="1" '
        f'stroke-dasharray="3,2" opacity="0.65"/>'
        f'<text fill="{PAL["T_t"]}" font-size="11" font-weight="600" font-style="italic" '
        f'text-anchor="middle" x="{sc_x + BW // 2}" y="{y_r + BH // 2 + 4}">T</text>'
    )
    L.append(
        f'<text fill="{PAL["orange"]}" font-size="7.5" text-anchor="middle" '
        f'x="{sc_x + BW // 2}" y="{y_r - 11}">soft-clip</text>'
    )

    # Labels — brace covers only the aligned portion (excludes soft-clipped T)
    L.append(brace_below(X0 + 5 * BW, X0 + 14 * BW, y_r + BH + 1, PAL["muted"],
                          "read extends past CPA into A-tract (del error; T soft-clipped)"))

    # Apparent 3' end = last ALIGNED base (right edge of last A, before soft-clip)
    app_x = X0 + 14 * BW
    L.append(vert_marker(app_x, y_r - 24, y_r + BH + 4, PAL["red"], "apparent 3' end", "right"))
    ...
    L.append(h_arrow(arrow_r, arrow_l, y_arrow, PAL["blue"],
                     "walk back: skip A's, deletions -- stop at C"))
...
def fig_multi_aligner_consensus():
    ...
    # Arrow from consensus to consensus BAM output
    y_out_arr = y_chain + pri_h + 10
    L.append(v_arrow(FIG_W/2, y_out_arr, y_out_arr + 18, PAL["green"]))

    # Output: consensus BAM (with XA/XC/XN tags — winner selected per read)
    y_out = y_out_arr + 18
    out_w = 220
    out_x = (FIG_W - out_w) / 2
    L.append(tsv_box(out_x, y_out, out_w, TSV_H + 2, "consensus.bam (winner per read)", PAL["green"]))

    # XA/XC/XN tags annotation
    L.append(f'<text fill="{PAL["muted"]}" font-size="8" font-style="italic" '
             f'x="{out_x + out_w + 10}" y="{y_out + (TSV_H+2)/2 + 3}">'
             f'tags: XA (winner aligner), XC (confidence), XN (agreement)</text>')

    # Arrow to rectify correct
    y_corr_arr = y_out + TSV_H + 2 + 10
    L.append(v_arrow(FIG_W/2, y_corr_arr, y_corr_arr + 16, PAL["blue"]))

    # rectify correct runs ONCE post-consensus on the winning BAM
    y_corr = y_corr_arr + 16
    corr_w = 220
    corr_x = (FIG_W - corr_w) / 2
    L.append(cmd_box(corr_x, y_corr, corr_w, CMD_H, "rectify correct  (once, post-consensus)", PAL["blue"]))

    # Side annotation — what rectify correct does
    ann2_x = FIG_W - 12
    L.append(f'<text fill="{PAL["muted"]}" font-size="7.5" font-style="italic" '
             f'text-anchor="end" x="{ann2_x}" y="{y_corr + 8}">3\u2032 walk-back</text>')
    L.append(f'<text fill="{PAL["muted"]}" font-size="7.5" font-style="italic" '
             f'text-anchor="end" x="{ann2_x}" y="{y_corr + 18}">5\u2032 junction rescue</text>')
    L.append(f'<text fill="{PAL["muted"]}" font-size="7.5" font-style="italic" '
             f'text-anchor="end" x="{ann2_x}" y="{y_corr + 28}">soft-clip rescue</text>')

    # Arrow to final TSV output
    y_tsv_arr = y_corr + CMD_H + 10
    L.append(v_arrow(FIG_W/2, y_tsv_arr, y_tsv_arr + 16, PAL["blue"]))

    # Final corrected_3ends.tsv output
    y_final = y_tsv_arr + 16
    L.append(tsv_box(corr_x, y_final, corr_w, TSV_H + 2, "corrected_3ends.tsv", PAL["blue"]))

    # ════════════════════════════════════════════════════════════════
    # STAGE 3 — Chimeric reconstruction (optional)
    # ════════════════════════════════════════════════════════════════
    y_div2 = y_final + TSV_H + 2 + 16
...
def fig_polya_pretrim():
    """
    Illustrates the three-pass poly(A) + adapter detection algorithm from
    drs_trim_command.py.  Shows three example reads — one per pass type —
    all producing the same trimmed output.  The strict-mode poly(A) scan
    (default max_error_rate=0.0) stops at the first non-A base, leaving
    ambiguous bases for the post-alignment walk-back step.
    """
    H = 420
    X0 = 70
    BODY_BLK_W = 120
    bx = X0 + BODY_BLK_W + 4   # where individual bases start
    ...
    # ── Helper: draw a colored base row ──
    def colored_bases(x, y, seq, fill, border, text_c, fade=False):
        parts = []
    ...
    def amb_base(x, y, b="T"):
        return (
            f'<rect fill="{AMB_F}" height="{BH}" rx="{BR}" width="{BW}" '
            f'x="{x}" y="{y}" stroke="{AMB_B}" stroke-width="1.2"/>'
            f'<text fill="{AMB_B}" font-size="11" font-weight="700" text-anchor="middle" '
            f'x="{x + BW//2}" y="{y + BH//2 + 4}">{b}</text>'
        )

    # ── Shared sequence layout ──
    # All three reads share a prefix: [body block] G A A A [T_amb]
    # After the ambiguous T: 5 poly(A) A's + varying adapter stubs
    prefix = list("GAAA")
    n_pre = len(prefix)
    bx_t = bx + n_pre * BW          # ambiguous T position
    bx_pa = bx_t + BW               # poly(A) A's start
    polya = list("AAAAA")
    n_pa = len(polya)
    bx_stub = bx_pa + n_pa * BW     # adapter stub starts

    def draw_body_prefix(y):
        """Body block + GAAA prefix + ambiguous T."""
        parts = []
        parts.append(aligned_block(X0, y, BODY_BLK_W, BH, "transcript body", BODY_C))
        for i, b in enumerate(prefix):
            parts.append(base(bx + i*BW, y, b))
        parts.append(amb_base(bx_t, y))
        return "\n".join(parts)

    def strikethrough(x1, x2, y_mid, color=PAL["red"], opacity="0.6"):
        return (f'<line stroke="{color}" stroke-width="2" '
                f'x1="{x1}" x2="{x2}" y1="{y_mid}" y2="{y_mid}" opacity="{opacity}"/>')

    # ════════════════════════════════════════════════════════════════
    # Section 1: THREE-PASS DETECTION
    # ════════════════════════════════════════════════════════════════
    y_sec = 42
    L.append(section_head("THREE-PASS POLY(A) + ADAPTER DETECTION  (RNA 5\u2032 \u2192 3\u2032)", y_sec))
    L.append(f'<text fill="{PAL["muted"]}" font-size="8" font-style="italic" '
             f'x="60" y="{y_sec + 13}">strict mode (default): scan stops at first non-A base</text>')

    # ── Pass 0: No adapter stub — Dorado already removed it ──
    y_p0 = y_sec + 32
    L.append(f'<text fill="{PAL["green"]}" font-size="9" font-weight="700" '
             f'x="14" y="{y_p0 + BH//2 + 4}">Pass 0</text>')
    L.append(draw_body_prefix(y_p0))
    L.append(colored_bases(bx_pa, y_p0, polya, POLYA_F, POLYA_B, POLYA_B))
    # Strikethrough on poly(A) only
    L.append(strikethrough(bx_pa - 2, bx_pa + n_pa*BW + 2, y_p0 + BH//2))
    # Scan arrow above poly(A) — leftward from 3' end
    arr_y = y_p0 - 7
    L.append(h_arrow(bx_pa + n_pa*BW - 6, bx_t + BW + 4, arr_y, POLYA_B,
                      "scan \u2190 (pure A only)"))
    # Stop bar at ambiguous T
    L.append(f'<line stroke="{AMB_B}" stroke-width="2" '
             f'x1="{bx_t + BW + 1}" x2="{bx_t + BW + 1}" '
             f'y1="{y_p0 - 2}" y2="{y_p0 + BH + 2}"/>')
    # 5'/3' labels
    L.append(f'<text fill="{PAL["muted"]}" font-size="8" x="{X0 - 16}" '
             f'y="{y_p0 + BH//2 + 3}">5\u2032</text>')
    L.append(f'<text fill="{PAL["muted"]}" font-size="8" x="{bx_pa + n_pa*BW + 5}" '
             f'y="{y_p0 + BH//2 + 3}">3\u2032</text>')
    # Annotation
    ax0 = bx_pa + n_pa*BW + 18
    L.append(f'<text fill="{PAL["label"]}" font-size="8" '
             f'x="{ax0}" y="{y_p0 + BH//2 + 3}">Dorado already removed adapter</text>')

    # ── Pass 1: Regex-detected adapter stub ──
    y_p1 = y_p0 + BH + 24
    L.append(f'<text fill="{ADAPTER_B}" font-size="9" font-weight="700" '
             f'x="14" y="{y_p1 + BH//2 + 4}">Pass 1</text>')
    L.append(draw_body_prefix(y_p1))
    L.append(colored_bases(bx_pa, y_p1, polya, POLYA_F, POLYA_B, POLYA_B))
    adapter1 = list("TCCT")
    n_a1 = len(adapter1)
    L.append(colored_bases(bx_stub, y_p1, adapter1, ADAPTER_F, ADAPTER_B, ADAPTER_B))
    # Strikethrough on poly(A) + adapter
    L.append(strikethrough(bx_pa - 2, bx_pa + n_pa*BW + 2, y_p1 + BH//2))
    L.append(strikethrough(bx_stub - 2, bx_stub + n_a1*BW + 2, y_p1 + BH//2, ADAPTER_B))
    # Brace below adapter
    L.append(brace_below(bx_stub, bx_stub + n_a1*BW, y_p1 + BH + 1, ADAPTER_B,
                          "adapter stub"))
    # Annotation
    ax1 = bx_stub + n_a1*BW + 10
    L.append(f'<text fill="{PAL["label"]}" font-size="8" '
             f'x="{ax1}" y="{y_p1 + BH//2 - 1}">regex T[CT]{{0,10}}$</text>')
    L.append(f'<text fill="{PAL["label"]}" font-size="8" font-style="italic" '
             f'x="{ax1}" y="{y_p1 + BH//2 + 10}">strips adapter, then scans A\u2019s</text>')

    # ── Pass 2: Iterative peel (adapter has basecall errors) ──
    y_p2 = y_p1 + BH + 44
    L.append(f'<text fill="{PAL["red"]}" font-size="9" font-weight="700" '
             f'x="14" y="{y_p2 + BH//2 + 4}">Pass 2</text>')
    L.append(draw_body_prefix(y_p2))
    L.append(colored_bases(bx_pa, y_p2, polya, POLYA_F, POLYA_B, POLYA_B))
    adapter2 = list("TCG")
    n_a2 = len(adapter2)
    L.append(colored_bases(bx_stub, y_p2, adapter2, ADAPTER_F, ADAPTER_B, ADAPTER_B))
    # Strikethrough on poly(A) + adapter
    L.append(strikethrough(bx_pa - 2, bx_pa + n_pa*BW + 2, y_p2 + BH//2))
    L.append(strikethrough(bx_stub - 2, bx_stub + n_a2*BW + 2, y_p2 + BH//2, ADAPTER_B))
    # Brace below adapter
    L.append(brace_below(bx_stub, bx_stub + n_a2*BW, y_p2 + BH + 1, ADAPTER_B,
                          "error stub"))
    # Annotation
    ax2 = bx_stub + n_a2*BW + 10
    L.append(f'<text fill="{PAL["label"]}" font-size="8" '
             f'x="{ax2}" y="{y_p2 + BH//2 - 1}">iterative peel (rescue)</text>')
    L.append(f'<text fill="{PAL["label"]}" font-size="8" font-style="italic" '
             f'x="{ax2}" y="{y_p2 + BH//2 + 10}">stub has G \u2014 breaks regex</text>')

    # Small "?" above ambiguous T in output row (avoids overlap with scan arrow)
    # The annotation in the output section explains the ambiguity

    # ════════════════════════════════════════════════════════════════
    # Section 3: TRIMMED OUTPUT (same for all three passes)
    # ════════════════════════════════════════════════════════════════
    y_div = y_p2 + BH + 28
    L.append(hdivider(y_div))
    y_sec3 = y_div + 14
    L.append(section_head("TRIMMED OUTPUT (for re-alignment)", y_sec3, PAL["green"]))

    y_out = y_sec3 + 18
    L.append(row_label("Output", y_out + BH//2 + 4, x=14))
    # Body block + prefix + ambiguous T (all kept)
    L.append(draw_body_prefix(y_out))
    # "seq error or genomic?" annotation above the T
    L.append(f'<text fill="{AMB_B}" font-size="7" font-weight="600" '
             f'text-anchor="middle" x="{bx_t + BW//2}" y="{y_out - 5}">'
             f'seq error or genomic?</text>')
    # Checkmark + note
    note_x = bx_t + BW + 10
    L.append(f'<text fill="{PAL["green"]}" font-size="10" font-weight="700" '
             f'x="{note_x}" y="{y_out + BH//2 + 0}">'
             f'\u2713 ready for alignment</text>')
    L.append(f'<text fill="{PAL["muted"]}" font-size="8" font-style="italic" '
             f'x="{note_x}" y="{y_out + BH//2 + 12}">'
             f'ambiguous T resolved by 3\u2032 walk-back after alignment</text>')

    # Metadata note
    L.append(f'<text fill="{PAL["muted"]}" font-size="8" font-style="italic" '
             f'text-anchor="middle" x="{FIG_W//2}" y="{y_out + BH + 22}">'
             f'poly(A) length, adapter sequence, and pass number saved to metadata</text>')
