...
```
The `NO` means those dirs accumulated chunks from **multiple/superseded Xrn1_repB attempts** (173 dirs, `_of_039`),
so they hold duplicate/extra reads not in the final merged. Almost certainly disposable stale scratch — but not
*provably a subset*, so not eligible for the "verified-redundant" auto-reclaim standard I used on the decay run.
