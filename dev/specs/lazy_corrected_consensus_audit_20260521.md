...
| Phase E fixes implementation | ✅ DONE — commit `f7e6463` (2026-05-22) | Two fixes: (1) `_stage_raw_bams` context manager copies raw BAMs to `$L_SCRATCH`/`$SCRATCH` NVMe before `merge_corrected_tsvs` + `write_corrected_consensus_bam`; wired into `split_command.py` chunk-merge template and both `single_sample.py` call sites. (2) `pool.shutdown(wait=False)` lets 22 s ProcessPool queue-drain overlap with post-scoring merge work. All 1263 tests pass. |
| Phase F — NVMe staging validation on Sherlock | ⏳ PENDING | Pull `f7e6463` onto Sherlock, submit timing job (reuse `phase_e_parallel_timing_20260522.sh` structure, sections a+b only). Expected: total ≈ 83–100 s. See `dev/specs/lazy_consensus_timing_20260521.md#phase-f` for full script spec and interpretation guide. |
