...
The panel's fourth arm is RECTIFY's own **overhang resolver** — a native re-alignment stage (Station A of the re-aligner; no external binary) that consumes the finished minimap2 BAM and substitutes the resolved BAM for the raw minimap2 arm. By volume its largest measured job is **junction re-arbitration**: re-scoring the aligner's own junction calls anywhere in the CIGAR against the splice-site index (planning/720, 897k cDNA reads: 24,686 junctions rewritten vs. 2,491 clip placements — roughly 10× the clip volume). **Terminal-clip placement** — the resolver's smaller, original job — re-places a soft-clipped overhang across a splice junction, the idea [gapmm2](https://github.com/nextgenusfs/gapmm2) inspired. Most of these moves run through one shared, deliberately upgraded engine (see `docs/ARCHITECTURE.md` for the exact per-family split):

- the search window for each overhang or junction is **bounded by its information content** — an explicit false-discovery budget instead of a fixed genomic window;
...
- candidate sites come from a **precomputed splice-site index** (binary-search range query, never a scan), with configurable acceptor classes for alternative-3'SS studies (`--resolver-acceptor-classes prp18`) and a paired AT-AC intron class enabled by default (yeast splices AT-AC through its *major* spliceosome and in human it is the U12-type class, so every eukaryote has them; `--no-resolver-atac` reproduces the pre-2026-09-05 candidate space). The per-clip candidate budget scales with the information-bounded search window, so a mammalian `--max-intron` is searched rather than silently abandoned, and the homopolymer-aware scoring kernel is compiled by default (`RECTIFY_HP_ED_NUMBA=0` opts out) because that search is ~20× cheaper with it;
...
- the stage runs on **`-t N` worker processes** (batches of records on a pool, one writer keeping input order, output byte-identical to `-t 1`), and every clip it **abandons on the per-clip candidate ceiling is reported as a correctness metric** — count, fraction and per-contig split in `<prefix>.overhang_resolver.stats.json`, with the end-of-run warning naming the consequence (each abandoned clip is a junction rescue that did not run). Raise the ceiling with `--resolver-candidate-ceiling N` when that fraction is non-zero and the numba kernel is available.
