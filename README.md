...
The panel's fourth arm is RECTIFY's own **overhang resolver** — a native re-alignment stage (Station A of the re-aligner; no external binary) that consumes the finished minimap2 BAM and substitutes the resolved BAM for the raw minimap2 arm. By volume its largest measured job is **junction re-arbitration**: re-scoring the aligner's *own* junction calls anywhere in the CIGAR — including interior junctions — against the same splice-site index, by merging a stray boundary deletion into an abutting intron, shifting a donor/acceptor to a better-scoring nearby site, snapping a stand-alone intron-length deletion onto an indexed site, or re-splicing a linear stretch where a mismatch cluster betrays a missed junction (Chanfreau planning/720, 897k cDNA reads: boundary-deletion merges alone rewrote 24,686 junctions vs. 2,491 clip placements — roughly 10× the clip volume). **Terminal-clip placement** — re-placing a soft-clipped overhang across a splice junction — is the resolver's smaller, original job, and the one [gapmm2](https://github.com/nextgenusfs/gapmm2) inspired with its insight that a minimap2 alignment can be post-processed to recover terminal splice overhangs. Most of these moves — clip placement, boundary shifts, and mismatch-cluster rescue — run through one shared, deliberately upgraded engine (the boundary-deletion merge and stand-alone-deletion snap use simpler, local checks instead; see `docs/ARCHITECTURE.md` for the exact split):
