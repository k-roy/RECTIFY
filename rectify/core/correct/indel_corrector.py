...
def rescue_softclip_at_homopolymer(
    ...
    if use_right_side:
        ...
        # NOTE: a "2-bp deletion extension (cat2_minus_2 motif rescue)" block
        # used to live here. It hard-coded the 2026-05-18 directive that landed
        # cat2_minus_2's 3' end on the genomic TTGC motif at chrI:128096 — a
        # genomic T, i.e. an RNA A on the minus strand. That violates the
        # 100%-non-A DRS policy (AGENT_FIXES [2026-06-13]): the pre-trim removes
        # the poly-A tail, so no corrected 3' end may sit on a stop base. The
        # directive also originated from a +strand case and was mis-applied to
        # this -strand read after the validation bundle was regenerated. Removed
        # 2026-06-13; the strip-trailing-T's guard above plus the poly-A
        # walkback (Module 2E) now land the read on the first genuine non-A.
