...
SR_ALIGN_CORRECT_TIME   = '6:00:00'   # per-chunk 7-aligner index loads dominate; non-preemptible partition
...
Examples:
  ...
  rectify split reads.fastq.gz --dry-run
    ...
    parser.add_argument(
        '--read-length', dest='read_length', type=int, default=150,
        help=(
            'Read length for the paired short-read COMPASS panel (STAR sjdbOverhang '
            '/ index selection). Default 150.'
        ),
    )
    parser.add_argument(
...
def _paired_short_read_array_body(
    n_chunks: int,
    sample_prefix: str,
    output_dir: Path,
    genome: str,
    ...
    """Per-chunk paired short-read body: COMPASS panel align → consensus, NO correction.

    A single ``rectify align --short-read --read2`` runs the full COMPASS panel
    (bbmap + STAR×2 + HISAT2×2 + magicblast + gsnap, paired) and the RN-keyed
    consensus selection with the COMPASS tiebreak — correction is never invoked
    (it is a separate stage). The per-chunk rectified BAM is copied to
    chunk_outputs/ as ``*.consensus.bam`` so the shared final-merge collects it.

    Idempotent: a present chunk output is skipped (preemption-safe).
    """
    shim = _TASK_ID_SHIM.replace('__CORES__', str(SR_ALIGN_CORRECT_CORES))
    limits = _thread_limits_block()
    n_pad = f'{n_chunks:03d}'
    return f"""#!/bin/bash
# RECTIFY — paired short-read per-chunk pipeline (COMPASS panel → consensus)
# Generated by: rectify split --generate-slurm --short-read --read2
# Panel: bbmap + STAR_default/noncanonical + HISAT2_default/noncanonical + magicblast + gsnap
# {n_chunks} tasks, one per paired chunk. NO 5'/3' end-correction (short-read junction mode).
#
# ── I/O POLICY ────────────────────────────────────────────────────────────────
# All aligner/consensus BAMs write to $SCRATCH (per-task scratch dir).
# Only the per-chunk consensus BAM is copied to {output_dir}/chunk_outputs/.
# DO NOT redirect intermediate BAM writes to Oak/NFS — it is ~75x slower.
# ──────────────────────────────────────────────────────────────────────────────
...
CHUNK_PAD=$(printf "%03d" "$RECTIFY_TASK_ID")
PREFIX="{sample_prefix}_chunk_${{CHUNK_PAD}}_of_{n_pad}"
R1="$OUTDIR/${{PREFIX}}_R1.fastq.gz"
R2="$OUTDIR/${{PREFIX}}_R2.fastq.gz"

CHUNK_OUT_DIR="$OUTDIR/chunk_outputs"
mkdir -p "$CHUNK_OUT_DIR"
CONSENSUS_OUT="$CHUNK_OUT_DIR/${{PREFIX}}.consensus.bam"

# ── Idempotent skip (preemption-safe) ───────────────────────────────────────
if [[ -f "$CONSENSUS_OUT" ]]; then
    echo "Chunk already complete, skipping: $CONSENSUS_OUT"
    exit 0
fi
...
[ -f "$R1" ] || {{ echo "ERROR: R1 chunk FASTQ not found: $R1" >&2; exit 1; }}
[ -f "$R2" ] || {{ echo "ERROR: R2 chunk FASTQ not found: $R2" >&2; exit 1; }}

cd "$RECTIFY_SRC"

# ── COMPASS panel align + RN-keyed consensus (tiebreak=compass, no correction) ──
echo ""
echo "=== rectify align --short-read --read2 (COMPASS panel + consensus) ==="
time $PYTHON -m rectify align \\
    "$R1" \\
    --read2 "$R2" \\
    --short-read \\
...
def _generate_paired_short_read_scripts(
    args: argparse.Namespace,
    n_chunks: int,
    sample_prefix: str,
) -> None:
    """Generate SLURM scripts for the paired short-read COMPASS pipeline.

    Per-chunk array (COMPASS panel align → consensus, no correction) + the shared
    final-merge (reused unchanged — it collects chunk_outputs/*.consensus.bam).
    """
    output_dir = args.output_dir.resolve()
    log_dir = output_dir / 'logs'
    log_dir.mkdir(parents=True, exist_ok=True)
    ...
    array_body = _paired_short_read_array_body(
        n_chunks, sample_prefix, output_dir, genome_str, annot_str,
        python_path, rectify_src, read_length=read_length,
    )
    array_body = array_body.replace(_SCHEDULER_HEADER_PLACEHOLDER_SR_ARRAY, array_headers)
    array_script_path = _write(output_dir / 'run_array_short_read.sh', array_body)

    # ── Final merge script (shared with single-end short-read) ────────────────
    merge_headers = _scheduler_headers(
        scheduler, f'{sample_prefix}_sr_merge', 1,
        SR_FINAL_MERGE_CORES, SR_FINAL_MERGE_MEM_GB, SR_FINAL_MERGE_TIME,
        str(log_dir), log_pat_job, is_array=False, **sched_kwargs,
    ...
    logger.info("Generated paired short-read COMPASS scripts (%s):", scheduler.upper())
    logger.info("  Array (COMPASS panel + consensus): %d tasks, %d cores, %dG, %s",
                n_chunks, SR_ALIGN_CORRECT_CORES, SR_ALIGN_CORRECT_MEM_GB, array_script_path)
    logger.info("  Final merge:                       1 job,   %d cores, %dG, %s",
                SR_FINAL_MERGE_CORES, SR_FINAL_MERGE_MEM_GB, merge_script_path)
    logger.info("  Oak output dir:                    %s", oak_str)
...
def _run_split_paired(args: argparse.Namespace) -> int:
    ...
    if getattr(args, 'generate_slurm', False):
        _generate_paired_short_read_scripts(args, n_chunks, prefix)
