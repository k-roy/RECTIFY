...
Off-the-shelf aligners often misplace both 5' and 3' ends of poly(A)-RNA reads: 5' overhangs that are soft-clipped instead of matched to an upstream exon, splice junctions that are forced to annotated junctions when an alternative intron is a better match, and poly(A) tails that align over genomic A-tracts so the apparent 3' end overshoots the true mature 3' end, sometimes by thousands of bp with an artifactual intron added. **RECTIFY corrects each of these in one pass** so the bases at the read ends mean what biology says they should — 5' end → transcription start site (for cDNA), N-cigar op → splice junction, 3' end → cleavage and polyadenylation (CPA) site/mature 3' end. To address the inherent ambiguity of CPA sites residing within A-tracts, RECTIFY takes the conservative approach of shifting all 3' ends to the first non-A residue.
...
| Poly(A) in read?      | Yes                    | Yes                                     | No                         |
| 3' end side of read   | right                  | right                                   | left                       |
| Gene strand vs BAM    | matches                | matches                                 | **inverted (antisense)**   |

All three converge at the multi-aligner and correction stages.
...
Nanopore reads present non-genomic sequence that can result in aligners misplacing the exact 5' and 3' ends of the RNAs due to basecalled poly(A), template-switching primers, UMIs, and adapter stubs. Pre-processing isolates the mRNA body for both DRS and cDNA so every downstream step sees the same kind of input.
...
`rectify align` runs **minimap2 + mapPacBio + gapmm2 + uLTRA + deSALT** in parallel for long reads, or **bbmap + bwa** for short reads (`--short-read`). Different aligners can disagree on junctions within reads for several reason, including inherently ambiguous placement due to identical sequence upstream or downstream of the donor and acceptor splice sites — running them in parallel gives RECTIFY a panel of candidate alignments per read to reconcile in the consensus step.

<p align="center">
  <img src="docs/figures/multi_aligner_consensus.png" alt="Multi-aligner consensus" width="720">
</p>

### (c) `rectify correct` — repair the read ends

Three independent rescues run per aligned read.

#### 5' ends — junction rescue
Long reads spanning splice junctions often carry soft-clipped 5' bases that match the upstream exon — the aligner couldn't extend through the junction. RECTIFY identifies these reads and extends the alignment to the best matching exon from the pool of junctions.
...
Each N-cigar op is refined against annotated donor/acceptor sites with homopolymer-aware scoring, then classified per read:
...
Scan the alignment from the 3' end inward. Walk past every A — whether the genome at that position is also A or not — and stop at the first non-A base where read matches reference. This is the algorithmic heart of RECTIFY: it catches **internal priming** at genomic A-tracts, where the read's own poly(A) tail aligns over genomic A's and the apparent 3' end overshoots the true cleavage site.
...
The three per-aligner corrected alignments are reconciled per read by priority (5' rescued → confidence → 3' agreement → span → junction count). Optional chimeric reconstruction stitches complementary junction sets across aligners to recover junctions no single aligner found. Output: one BAM where every record has been independently rescued, scored, and reconciled.
